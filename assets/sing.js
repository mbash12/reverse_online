...
function load(){
    if(lyrics != {}){
        if(lyrics["data"]["backgroundImage"] !== "" && lyrics["data"]["backgroundImage"]){
            $(".wrapper").css("background-image","url('"+path.replace(/\\/g, "/")+"songlist/"+song+lyrics["data"]["backgroundImage"]+"')");
        }else{
            var bg = "white";
            if(lyrics["data"]["backgroundColor"] !== "" && lyrics["data"]["backgroundColor"]){
...
function go(pos){
    ...
    var words = getClosest(Object.entries(lyrics["lyrics"][pxc]["lyrics"]), current);
    setActive(words);

    $("#timemil").html(current);
    // console.log(current);
    $("#time").html(current.substr(0, 5))
    $("#time1").html(current.substr(0, 5))
...
function goTo(position){
    ...
    var words = getClosest(Object.entries(lyrics["lyrics"][sentences]["lyrics"]), current);
    setActive(words);
}
        ...
        if(word.length > 0){
            ...
            word.forEach(e => {
                if(e[1] == "<br>"){
                    ...
                    // console.log(e[1]);
                    if(e[1] == "." || e[1] == " ."){
                        sentence += "<span class='big' id=\""+e[0].replace(":","").replace(".","")+"\">"+e[1].replace(" ","&nbsp;")+"</span>";
                    }else{
                        sentence += "<span id=\""+e[0].replace(":","").replace(".","")+"\">"+e[1].replace(" ","&nbsp;")+"</span>";
                    }
                }
            });
            $("#lyric").html(sentence);
...
function pdfBtn(){
    ...
    window.open(lyrics["data"]["pdf"], '_blank', 'height=570,width=520,scrollbars=yes');
}
function videoBtn(){
    $("#modal").slideToggle();
    ...
    if(lyrics["data"]["vsource"] == "online"){
        ...
        $("#modal iframe").hide();
        $("#modal video").attr('src', lyrics["data"]["video"]);
    }
}
function close(){
    $("#modal").slideToggle();
